...
export function cmdRender(a) {
  ...
  const propsPath=path.join(proj,'out',`props_${g.format.replace(':','x')}_${g.platform}.json`);const provided=a.props?readJson(path.resolve(a.props)):{};if(!provided || typeof provided!=='object'||Array.isArray(provided))die('invalid --props JSON');writeJson(propsPath,{...provided,format:g.format,platform:g.platform}); const gl = a.gl || (process.platform === 'linux' && !fs.existsSync('/dev/dri') ? 'swangle' : 'angle');
