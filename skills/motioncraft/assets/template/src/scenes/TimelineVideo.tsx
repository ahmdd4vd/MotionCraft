...
const SceneBody:React.FC<{s:TimelineScene;p:TimelineProps;kind:'launch'|'tutorial';beats:number[];fps:number}>=({s,p,kind,beats,fps})=>{
 ...
 <Zone name="top"><Headline level="h1" items={words(name||s.role,Math.min(8,Math.round(duration*.06)))}/></Zone>
 <Zone name="center">
 {(s.role==='demo'||s.role==='step')?<Card at={6} w={Math.min(g.safe.width*.86,820)} h={Math.min(g.safe.height*.47,540)} title={kind==='launch'?(p.product||'Demo'):'Screen capture'}>
